...
const EditScout: React.FC = () => {
  ...
  const fetchScout = async () => {
    try {
      ...
      setError(err.response?.data?.message || "Failed to fetch scout data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess("Scout profile updated successfully!");
      setTimeout(() => navigate("/admin/scout"), 1500);
    } catch (err: any) {
      console.error("Error updating scout:", err);
      setError(err.response?.data?.message || "Failed to update scout profile");
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="p-6">
        <div className="flex items-center justify-center min-h-96">
          ...
          <span className="ml-2 text-gray-600">Loading scout data...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6">
      <div className="flex items-center gap-4 mb-6">
        <Button
          variant="outline"
          size="icon"
          onClick={() => navigate("/admin/scout")}
        >
          <ArrowLeft className="w-4 h-4" />
        </Button>
        <h2 className="text-2xl font-semibold">Edit Scout Profile</h2>
      </div>
      ...
      {id && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border p-6 mb-6">
          ...
          <UserActions userId={id} username={formData.username} />
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div>
            ...
            <Input
              ...
              onChange={handleChange}
              placeholder="Talent ID, Negotiation, ..."
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            ...
            <Input
              ...
              onChange={handleChange}
              placeholder="Football, Recruiting, ..."
            />
          </div>
        </div>
